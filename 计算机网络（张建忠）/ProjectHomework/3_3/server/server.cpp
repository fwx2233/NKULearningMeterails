...
// 这里改成1，现在要实现的是拥塞控制，需要每个都进行回复
static unsigned int divenum = 1; 
			...
			if (newGetChecksum(recvBuf, 1024)==0) {
				if (currentNum == seq) {
					...
					if (seq > currentNum) {
						...
						beginTime = clock();
						cout << "TO C\t接收到的数据包序号＞当前所需，返回需要的ack\n";
						cout << "check: " << int(sendBuf[0]) << "\tack: " << (currentNum - 1) << endl;
						continue;
					}
					else {
						// 这里是收到了之前收到过的包，要不然就暂时不回复了
						// ack错误，返回其他的ack
						sendBuf[1] = (currentNum-1 >> 16) & 0xFF;
						sendBuf[2] = (currentNum-1 >> 8) & 0xFF;
						sendBuf[3] = (currentNum-1) & 0xFF;
				...
				beginTime = clock();
				cout << "TO C\t接收到的数据包内容有损坏，返回需要的ack\n";
				cout << "check: " << int(sendBuf[0]) << "\tack: " << (currentNum-1) << endl;
				continue;
			}
	}
