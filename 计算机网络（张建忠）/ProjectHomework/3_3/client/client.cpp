...
char selfIP[16] = "10.130.114.85";

// 流量控制
static int WINDOW_SIZE = 50; // 设置最大的窗口大小
		...
		if (lastAck == (curr - 1)) {
			...
			for (tempCal; (tempCal < cwnd) && (curr < totalNum); tempCal++) {
				...
				cout << "当前发送数据包为：\nseq: " << curr << "\nlen高: " << int(sendPackage[curr][4]) << "\tlen低: " << int(sendPackage[curr][5])
					<< "\tchecksum: " << int(sendPackage[curr][0]) << endl << "ssthresh：" << ssthresh << "\tcwnd：" << cwnd << endl;
				curr += 1;
			}
		}
		...
		if ((newGetChecksum(recvBuf, 4) == 0)) {
			if (answerNum == (lastAck + 1)) {
				...
				if (sameAckTimes == 3) {
					...
					//cout << "接收剩下的ans，当前状态为：\ncwnd: " << cwnd << "\tnumber: " << RecvNumInThisTurn << endl;
					...
					//	}
					//	RecvNumInThisTurn += 1;
					//	cout<<"cwnd: " << cwnd << "\tnumber : " << RecvNumInThisTurn << endl;
					//}
...
int sendMain() {
	...
	//cout << "输入客户端（自己）的IP：\n";
	//getIP(IP);
	inet_pton(AF_INET, selfIP, &clientAddr.sin_addr.s_addr);
