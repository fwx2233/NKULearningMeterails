	...
	while (1) {
		...
		if (recv(LocalhostSocket, CrecvBuf, CBUF_SIZE, 0) < 0) {
			...
			if (isHello(CrecvBuf)) {
				...
				if(CrecvBuf[0] == 'I' && CrecvBuf[1] == 'n' && CrecvBuf[2] == 'p'){
					cout << CrecvBuf << endl;
				}
				else {
					if (CrecvBuf[0] == 'u') {
						...
						if (isServerQuit(CrecvBuf)) {
							cout << "Server has stopped this chat room, bye.\n";
							break;
						}
						else {
							if (isClientQuit(CrecvBuf)) {
...
int clientMain() {
	...
	char* sendName = new char[int(userName[0]) + 4];
	sendName[0] = 'n';
