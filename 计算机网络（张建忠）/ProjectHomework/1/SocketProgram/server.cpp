...
char hello[SBUF_SIZE] = "Sconnected! Here is a message that server wanna say hello.\0";
...
DWORD WINAPI recvMessage(LPVOID) {
	while (1) {
		...
		if (recv(ClientSocket, SrecvBuf, SBUF_SIZE, 0) < 0) {
			//cout << "Connect failed. Error code is " << SOCKET_ERROR << ".\n";
		}
		else {
			if (isRecvQuit(SrecvBuf)) {
				cout << "Client has stopped this session, bye.\n";
				Sconnected = false;
				//break;
			}
			else
...
int serverMain() {
	...
	while (1) {
		...
		ClientSocket = accept(ServerSocket, (SOCKADDR*)&ClientAddr, &snaddr);
		// 检查新创建的socket是否有效
