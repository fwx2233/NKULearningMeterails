...
static unsigned int divenum = 10; // 用来整除的书，表示收到这么多的包就回一个ack
...
static bool shakeHand() {
	...
	while (1) {
		...
		//// 这里是第一次的，需要检查ACK为0，num为0，shake1
		//if (p.checksum == getChecksum(p) && p.ACK == 0 && p.SYN==1 && p.num == 0 && p.flag == SHAKE1) {
		//	// 收到的是对的
		//	// 制作第二次握手的包的内容
		//	sendP.SYN = true;
		//	sendP.ACK = p.num + 1;
		...
		//	
		//	sendP.flag = SHAKE2;
		//	sendP.checksum = getChecksum(sendP);
		//	// 发送第二次握手的数据包
		//	sendto(serverSocket, (char*)&sendP, sizeof(sendP), 0, (SOCKADDR*)&clientAddr, sockaddrLen);
		//	cout << "第二次握手数据包发送：\n";
		//	packageOutput(sendP);

		//	// 记录下刚刚的ack的值，一会要检测的时候应该是这个值+1
		//	int lastSeq = p.num + 1;
		//	// 清空接收缓冲区
		//	memset(&recvBuf, 0, MAX_BUFFER_SIZE);

		//	// 等待接收第三次握手的数据包
		//	recvLen = recvfrom(serverSocket, recvBuf, MAX_BUFFER_SIZE, 0, (sockaddr*)&clientAddr, &sockaddrLen);
		//	if (recvLen <= 0) {
		//		cout << "接收数据包出现错误，长度<=0\n";
		//		// 继续进行等待
		//		continue;
		...
		//		// 需要检查ACK，num为1，shake3
		//		if (p.checksum == getChecksum(p) && p.ACK == lastSeq && p.SYN==1 && p.num == 1 && p.flag == SHAKE3) {
		//			// 收到的是对的
		//			cout << "连接建立成功\n";
		//			return true;
		//		}
...
static void recvFile() {
	...
	// 对名字进行处理
	char filePath[] = "C:\\Users\\Nirvana\\Desktop\\test\\";
	char* fileNameN = new char[30 + nameLen + 1];
	for (int i = 0; i < 30 + nameLen; i++) {
		if (i < 30)
			fileNameN[i] = filePath[i];
		else {
			fileNameN[i] = fileName[i - 30];
		}
	}
	fileNameN[30 + nameLen] = '\0';
	...
	while (true) {
		while (recvfrom(serverSocket, recvBuf, 1024, 0, (sockaddr*)&clientAddr, &sockaddrLen) == SOCKET_ERROR) {
			...
			//cout << (currentNum == seq) << '\t' << currentNum << '\t' << seq << endl;
			if (newGetChecksum(recvBuf, 1024)==0) {
				if (currentNum == seq) {
					// 获取要读取的字符串的长度
					unsigned short int temp = unsigned short int(recvBuf[4]);
...
int serverMain() {
	...
	clientAddr.sin_port = htons(LUYOU_PORT);
