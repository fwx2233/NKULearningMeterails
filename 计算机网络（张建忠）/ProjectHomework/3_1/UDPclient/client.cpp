...
static void sayGoodBye2Server() {
	...
	//// 设置包中的一些内容
	//connectPackage.FIN = 1;
	//connectPackage.len = 0;
	//connectPackage.num = seq++;
	//// 由于是建立连接，并且现在是第一次，所以要交将flag的值设置为SHAKE1
	//connectPackage.flag = WAVE1;
	//connectPackage.checksum = getChecksum(connectPackage);

	//// 发送shake1的数据包
	//sendto(sendSocket, (char*)(&connectPackage), sizeof(connectPackage), 0, (SOCKADDR*)&serverAddr, socketAddrLen);
	//cout << "准备断开连接，数据包为（第一次挥手）：\n";
	//packageOutput(connectPackage);

	//while (true) {
	//	// 循环，等待来自服务器的回复
	//	int recvLen = recvfrom(sendSocket, recvBuffer, MAX_BUFFER_SIZE, 0, (SOCKADDR*)&serverAddr, &socketAddrLen);
	//	if (recvLen <= 0) {
	//		// 说明收到的包有问题，需要重传
	//		cout << "接收到的数据包出现问题，重新发送\n";
	//		sendto(sendSocket, (char*)(&connectPackage), sizeof(connectPackage), 0, (SOCKADDR*)&serverAddr, socketAddrLen);
	//	}
	//	else {
	//		// 收到了包，但是需要确定是不是我需要的包
	//		// 创建一个空的包，用来检查
	//		Package acceptPackage;
	//		memset(&acceptPackage, 0, sizeof(acceptPackage));
	//		// 将收到的内容赋给临时的包
	//		acceptPackage = *((Package*)recvBuffer);
	//		cout << "收到第二次挥手的数据包\n";
	//		packageOutput(acceptPackage);

	//		// 对包内的内容进行解析
	//		if (acceptPackage.ACK == seq && acceptPackage.FIN && acceptPackage.checksum == getChecksum(acceptPackage) && acceptPackage.flag == WAVE2) {
	//			// 检查ACK，SYN，flag和checksum，如果都对的话就是对的，否则重发刚刚的数据包
	//			// 没问题，对的
...
int sendMain() {
	...
	// ************输入自己（client）的IP
	cout << "输入客户端（自己）的IP：\n";
	char ClientIP[16] = "10.130.114.85";
	//getIP(IP);
