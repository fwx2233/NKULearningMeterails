...
%nterm <arrayinitstype> ArrayInits
%nterm <arraydeftype> ArrayDef
%nterm <stmttype> Stmts Stmt AssignStmt BlockStmt IfStmt ReturnStmt DeclStmt ConstDeclStmt VarDeclStmt ArrayDeclStmt FuncDef FuncParas FuncPara FuncDecl ExprStmt WhileStmt FuncCallParas FuncCallPara BREAKSTMT CONTINUESTMT
%nterm <exprtype> Exp AddExp Cond LOrExp PrimaryExp LVal RelExp LAndExp UnaryExp MulExp EqExp InitVal FuncCall  IOStream PutStream GetStream ArrayMember
%nterm <type> Type
...
ArrayDeclStmt
    :
    ID LBRACKT INTEGER RBRACKT {
        SymbolEntry *se;
        se = new IdentifierSymbolEntry(TypeSystem::intType, $1, identifiers->getLevel());
        dynamic_cast<IdentifierSymbolEntry*>(se)->setvar();
        identifiers->install($1, se);
        Id* tempid=new Id(se);
        $$=new ArrayDeclStmt(tempid,$3);
    }
    |
    ID LBRACKT INTEGER RBRACKT ASSIGN ArrayDef{
        SymbolEntry *se;
        se = new IdentifierSymbolEntry(TypeSystem::intType, $1, identifiers->getLevel());
        dynamic_cast<IdentifierSymbolEntry*>(se)->setvar();
        identifiers->install($1, se);
        Id* tempid=new Id(se);
        $$=new ArrayDeclStmt(tempid,$3,$6);
    }
    ;
