...
{
    auto funcse = dynamic_cast<FunctionDecl *>(this->functiondecl)->getSe();
...
void Def::genCode()
{
    ...
    if(arrayDeclStmt!=0)
    {
        dynamic_cast<ArrayDeclStmt*>(arrayDeclStmt)->genCode();
        return;
    }
...
void ArrayDeclStmt::genCode()
{
    ...
    if (se->isLocal())
    {
        ...
        addr_se = new TemporarySymbolEntry(type, SymbolTable::getLabel());
        addr = new Operand(addr_se);
        alloca = new ArrayAllocaInstruction(addr, se, n); // allocate space for local id in function stack.
        entry->insertFront(alloca);                       // allocate instructions should be inserted into the begin of the entry block.
        se->setAddr(addr);
    }
    if (this->def != 0)
    {
        ...
        for (int i = 0; i < def->getInits()->size(); i++)
        {
            SymbolEntry *constse = new ConstantSymbolEntry(TypeSystem::intType, (*(def->getInits()))[def->getInits()->size()-1-i]);
            Operand *Src = new Operand(constse);
            SymbolEntry* se=new ConstantSymbolEntry(TypeSystem::intType,i);
            new StoreInstruction(new Constant(se),addr, Src, bb);
        }
    }
}

void ArrayMember::genCode()
{
    BasicBlock *bb = builder->getInsertBB();
    Operand *addr = dynamic_cast<IdentifierSymbolEntry *>(symbolEntry)->getAddr();
    n->genCode();
    new LoadInstruction(dst, addr, n, bb);
}
...
void AssignStmt::genCode()
{
     ...
     */
    if(dynamic_cast<ArrayMember*>(lval))
    {
        dynamic_cast<ArrayMember*>(lval)->getArrayno()->genCode();
        new StoreInstruction(dynamic_cast<ArrayMember*>(lval)->getArrayno(),addr, src, bb);
    }
    else
    {
...
void funcParam::genCode()
{
    ...
    BasicBlock *bb;
    bb = builder->getInsertBB();
    new StoreInstruction(se->getAddr(), id->getOperand(), bb, paramno);
    paramno++;
}
...
void StmtNode::checkIsInCircle()
{
    if (!getisincircle())
    {
        fprintf(stderr, "ERROR:continue/break must be used in circle\n");
        check = 0;
    }
}
...
void BreakStmt::typeCheck(int level)
{
    fprintf(stdout, "%*c", level, ' ');
    fprintf(stdout, "Break stmt\n");
    checkIsInCircle();
}
...
void BreakStmt::genCode()
{
    Function *func = builder->getInsertBB()->getParent();
    BasicBlock *bb = builder->getInsertBB();
    // get end_bb, this program only has while stmt
    //BasicBlock *new_bb = new BasicBlock();

    new UncondBrInstruction(dynamic_cast<WhileStmt *>(this->while_stmt)->getEndbb(), bb);
    //new BreakInstruction(bb);
    BasicBlock *next_bb = new BasicBlock();
...
void ContinueStmt::typeCheck(int level)
{
    fprintf(stdout, "%*c", level, ' ');
    fprintf(stdout, "Return stmt\n");
    checkIsInCircle();
...
void ContinueStmt::genCode()
{
    ...
    new ContinueInstruction(dynamic_cast<WhileStmt *>(this->while_stmt)->getCondbb(), bb);
    //new ContinueInstruction(bb);
    BasicBlock *next_bb = new BasicBlock();
    builder->setInsertBB(next_bb);
}
