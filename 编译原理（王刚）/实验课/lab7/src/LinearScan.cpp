...
bool LinearScan::linearScanRegisterAllocation()
{
    ...
    for (auto unhandled_interval : intervals)
    {
        ...
        if(actives.size()==regs.size())
        {
            ...
            if(unhandled_interval->end>lastactive->end)
            {
            ...
            else
            {
                ...
                setReg(unhandled_interval,lastactive->rreg);
                eraseActive(lastactive);
                insertActive(unhandled_interval);
            }
            flag=false;
        }
        else
...
void LinearScan::insertActive(Interval *handled_interval)
{
    if (actives.size() == 0)
    {
