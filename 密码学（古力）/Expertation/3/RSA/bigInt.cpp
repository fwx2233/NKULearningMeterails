...
void BigInt::Random()
{
	for (int i = 0; i < (thesize / 4); i++) {
		if ((i == 3) || (i == (thesize / 4) - 1)) {
			// 使用之前的两位进行一个加密
			int tempHigh = data[i] - 2;
			int tempLow = data[i] - 1;
			data[i] = encryptForRSA(tempHigh, tempLow);
		}
		else {
			//由于RAND()最大只能产生0X7FFF的数,为了能产生32位的随机数,需要3次RAND()操作
...
// 返回大数长度
// 有点问题，这个返回的是int数组中不为0的长度，但是应该返回的是bit位的
